...
This image is designed to be run rootless and with a readonly rootfs. Generally, security has been taken seriously and no writes should occur outside of `/tmp/`.
...
```
docker pull crystalnetorg/powerdns-auth:<tag>
```

Run the PowerDNS container with default settings (no database backend):

```
docker run -d -p 53:10353/tcp -p 53:10353/udp crystalnetorg/powerdns-auth:<tag>
```
...
services:
  powerdns:
    image: crystalnetorg/powerdns-auth:<tag>
    ports:
      - "53:10353/udp"
      - "53:10353/tcp"
      - "8081:8081/tcp"
